      ...
      <div class="">
        ...
        <div class=" pt-2 bg-light">
          <div class="card shadow-sm p-4">
            ...
            <form method="POST" action="{{ route('car_valuation.store') }}" class="add-role-form">
              @csrf
              <div class="row g-3">
                <!-- Car Make -->
        ...
        <script>
          document.addEventListener("DOMContentLoaded", function () {
            ...
            function addRow(type, fromYear, toYear, value = '') {
              let targetContainer;
              let namePrefix = '';
              ...
              row.dataset.range = `${fromYear}-${toYear}`;

              let labelText = (fromYear === toYear) ? `Year ${fromYear}` : `Year ${fromYear} - ${toYear}`;

              row.innerHTML = `
              <label class="me-2">${labelText}</label>
              <div class="input-group w-auto">
                  <input type="text" 
                         name="${namePrefix}[${fromYear}-${toYear}]" 
                         value="${value}" 
                         class="form-control" 
                         placeholder="Enter Number">
                  <span class="input-group-text">%</span>
          ...
          `;
            ...
            @if(!empty($valuation->car_depreciation))
              let carDep = @json($valuation->car_depreciation);
              Object.entries(carDep).forEach(([range, value]) => {
                let [fromYear, toYear] = range.split("-").map(Number);
                addRow('car', fromYear, toYear, value);
              });
            @endif

              // --- Prefill Other Aging Depreciation ---
              @if(!empty($valuation->other_aging_depreciation))
                let agingDep = @json($valuation->other_aging_depreciation);
                Object.entries(agingDep).forEach(([range, value]) => {
                  let [fromYear, toYear] = range.split("-").map(Number);
                  addRow('aging', fromYear, toYear, value);
                });
              @endif
            ...
            document.getElementById("generateBtn").addEventListener("click", function (e) {
              ...
              addRow(type, fromYear, toYear); // Add new row with empty value
            });
